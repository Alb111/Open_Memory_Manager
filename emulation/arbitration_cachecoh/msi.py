...
Key Concepts:
- MSI States: INVALID (no copy), SHARED (read-only), MODIFIED (exclusive/dirty)
- Processor Events: CPU read (PR_RD) and write (PR_WR) requests
- Snoop Events: Coherence messages from other caches via the directory
- Coherence Commands: Bus transactions and snoop messages

Integration Points:
- CacheController uses on_processor_event() and on_snoop_event()
