"""
Cache Controller Module for MSI Cache Coherence Protocol

This module implements a cache controller that interfaces between the CPU and
the directory-based coherence system. Each cache controller manages local cache
storage and handles both processor requests and directory snoop messages.

Architecture:
    CPU → CacheController → DirectoryController → Memory
         ↑                         ↓
         └─── Snoop Messages ──────┘

Key Responsibilities:
1. Handle CPU read/write requests (mem_instr=False)
2. Manage cache line states using MSI protocol
3. Issue coherence commands to directory when needed
4. Respond to snoop messages from directory (mem_instr=True)
5. Perform cache line evictions with proper writebacks

Integration Points:
- Receives axi_request from CPU (normal memory traffic)
