...
//
// One 1024-deep x 32-bit memory bank, built from four 3.3V gf180mcu 1024x8
// single-port SRAMs (one per byte lane). This is the 1024-deep analog of
// mem_ctrl_512x32; the 1024x8 macro exists only in the OCD (3.3V) SRAM library,
// so this bank is 3.3V-only.

`default_nettype none

module mem_ctrl_1024x32
(
	input wire         clk_i,
	input wire         rst_ni,
  ...
  `endif
	...
	// addr written to or read from (1024 deep -> 10-bit)
	assign sram_addr = mem_addr_i[9:0];
	// data to be written
	...
	(* keep *)  gf180mcu_ocd_ip_sram__sram1024x8m8wm1 sram2 (
		.CLK(clk_i),
		.CEN(sram_enable_n),
	...
	(* keep *) gf180mcu_ocd_ip_sram__sram1024x8m8wm1 sram3 (
		.CLK(clk_i),
		.CEN(sram_enable_n),
