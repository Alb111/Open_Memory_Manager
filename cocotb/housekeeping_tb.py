...
FLASH_DATA = [
    ...
    0x55, 0x66, 0x77, 0x88,   
    0x99, 0xAA, 0xBB, 0xCC,
    0xDD, 0xEE, 0xFF, 0x19,   #(boot_size=32)
]
...
async def wait_for_boot_done(dut, timeout_cycles=200_000):
...
async def wait_for_n_writes(dut, n, timeout_cycles=200_000):
    write_count = 0
    for _ in range(timeout_cycles):
        await RisingEdge(dut.clk_i)
...
async def test_reset(dut):
    ...
    print(f"  mem_valid_o = {int(dut.mem_valid_o.value)}  (expected 0)")
    print(f"  cores_en_o = {int(dut.cores_en_o.value)}  (expected 0)")
    print(f"  boot_done_o = {int(dut.boot_done_o.value)}  (expected 0)")

    assert dut.mem_valid_o.value == 0,   "sram_wr_en should be 0 during reset"
...
async def test_full_boot_sequence(dut):
    ...
    for _ in range(200_000):
        await RisingEdge(dut.clk_i)
        if dut.mem_valid_o.value == 1:
            sram_writes.append((int(dut.mem_addr_o.value), int(dut.mem_wdata_o.value)))
        if dut.boot_done_o.value == 1:
            break
...
async def test_mux_passthrough_mode(dut):
    ...
    assert dut.mem_valid_o.value == 0, "sram_wr_en must stay 0 — no writes while in pass-through"
